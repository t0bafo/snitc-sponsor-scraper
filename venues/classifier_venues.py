...
def score_venue(venue: Dict, signals: Dict) -> int:
    ...
    if signals.get("has_rooftop"):
        score += cfg.PRIORITY_SCORE_RULES["has_rooftop"]
    if signals.get("has_private_events"):
        score += cfg.PRIORITY_SCORE_RULES["has_private_events"]
    if signals.get("has_bar_service"):
        score += cfg.PRIORITY_SCORE_RULES["has_bar_service"]
    if signals.get("has_views"):
        score += cfg.PRIORITY_SCORE_RULES["has_views"]
    if signals.get("has_indoor_backup"):
        score += cfg.PRIORITY_SCORE_RULES["has_indoor_backup"]
    if signals.get("has_nightlife"):
        score += cfg.PRIORITY_SCORE_RULES["has_nightlife"]
    if signals.get("has_afrocultural"):
        score += cfg.PRIORITY_SCORE_RULES["has_afrocultural"]
    ...
    if hood in cfg.PRIORITY_NEIGHBORHOODS.get(city, []):
        score += cfg.PRIORITY_SCORE_RULES["primary_location"]
    else:
        score += cfg.PRIORITY_SCORE_RULES["secondary_location"]

    cap_str = venue.get("capacity", "")
    if cap_str:
        try:
            cap = int(str(cap_str).split("-")[0])
            if cfg.CAPACITY_IDEAL_MIN <= cap <= cfg.CAPACITY_IDEAL_MAX:
                score += cfg.PRIORITY_SCORE_RULES["capacity_in_range"]
            elif cap >= cfg.CAPACITY_MIN:
                score += 1  # partial credit
        except ValueError:
            pass

    if venue.get("website"):
        score += cfg.PRIORITY_SCORE_RULES["has_website"]
    if venue.get("contact"):
        score += cfg.PRIORITY_SCORE_RULES["has_contact"]
...
def classify_priority(venue: Dict, signals: Dict) -> str:
    ...
    if s >= cfg.PRIORITY_HIGH_SCORE:
        return "Priority A"
    elif s >= cfg.PRIORITY_MED_SCORE:
        return "Priority B"
    else:
        return "Priority C"
